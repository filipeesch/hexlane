...
On every `op run`, `http call`, or `sql query`, hexlane:

1. **Looks up** the cached credential for the target identity in SQLite metadata
...
For credentials that are externally managed — long-lived JWTs, service account keys, direct DB passwords — declare the target with `acquire_strategy: static` and load the value once with `credential set`. hexlane stores it encrypted in the vault and tracks expiry automatically.

See [integration-config.md](integration-config.md#static-strategy) for how to declare a static credential.
...
For targets using `api_token` or `db_connection` credential kind with `http` or `shell` strategies, configure `renewal_policy` in the integration YAML:
...
- Static credentials (`acquire_strategy: static`) do not auto-renew. Rotation is manual via `credential set`.
